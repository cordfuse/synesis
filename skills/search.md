...
3. **Return results.** For each match, report:
   ...
   - Tags
   - Status for decisions (`proposed` / `accepted` / `rejected`; notes carry none), plus `superseded` where a `superseded-by` pointer is set
...
- If a record carries `superseded-by`, point the developer to its replacement. A `rejected` record with a pointer is a rejection that was later reversed — say so, since both halves matter.
