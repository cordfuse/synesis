...
1. **Recent activity.** List the last 5-10 records by `date` (most recent first). For each: title, date, decided-by, status.
...
4. **Stale items.** Check `records/` and `conventions/` for anything with `last-verified` older than `stale-days`. List any stale items as a heads-up.
